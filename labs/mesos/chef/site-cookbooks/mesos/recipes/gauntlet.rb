...
ruby_block "template validate.sh" do
  block do
    ...
    file.search_file_replace(/export MESOS_MASTER_URL.*/, "export MESOS_MASTER_URL=\"mesos://zk://#{node['mesos']['zk_servers'].split(',').join(':2181,')}:2181/mesos\"")
    file.search_file_replace(/export MESOS_EXECUTOR_URI.*/, "export MESOS_EXECUTOR_URI=\"#{node['mesos']['spark']['tarball_url']}\"")
